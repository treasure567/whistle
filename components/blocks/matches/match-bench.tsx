...
function BenchSide({
  ...
  return (
    <div>
      <div className="mb-3 flex items-center gap-2">
        ...
        <span
          className={cn(
            ...
            side === "home" ? "bg-violet-400" : "bg-zinc-400",
          )}
          aria-hidden
        />
      </div>
          ...
          <GlowCard key={p.id} padding="none">
            <div className="flex items-center gap-2.5 p-2.5">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-[12px] text-zinc-100">{p.name}</p>
      ...
      <Link
        href={`/teams/${nation}`}
        className="mt-3 inline-block font-mono text-[10px] uppercase tracking-[0.18em] text-zinc-500 hover:text-violet-300"
      >
        Full squad →
      </Link>
    </div>
