    ...
    <div className="flex flex-col gap-4 rounded-2xl border border-border bg-card p-5">
      <div>
        <div className="flex items-baseline justify-between">
          ...
          <span className={cn("font-mono text-sm tabular-nums", over ? "text-red-600 dark:text-red-300" : "text-foreground")}>
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {POSITIONS.map((position) => (
            <span
              key={position}
              className="rounded-sm border border-border px-2 py-1 font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground"
            >
              {position} {counts[position]}
            </span>
          ))}
          <span className="rounded-sm border border-border px-2 py-1 font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
          ...
          {!validation.valid && slots.length > 0 ? (
            <ul className="flex flex-wrap gap-1.5">
              {validation.errors.map((message) => (
                <li
                  key={message}
                  className="rounded-sm border border-amber-500/20 bg-amber-500/[0.05] px-2 py-1 font-mono text-[10px] uppercase tracking-[0.16em] text-amber-600 dark:text-amber-300"
                >
                  {message}
                </li>
              ))}
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2">
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm text-foreground">{slot.player.name}</p>
      ...
      <IconToggle
        ...
        activeClass="border-emerald-500/40 bg-emerald-500/[0.08] text-emerald-600 dark:text-emerald-300"
      >
        <StarIcon size={13} />
      </IconToggle>
      <IconToggle
        ...
        label="Make captain"
        activeClass="border-violet-400/50 bg-violet-500/[0.10] text-violet-500 dark:text-violet-300"
      >
        <CrownIcon size={13} />
      </IconToggle>
      <IconToggle onClick={onRemove} label="Remove player" hoverClass="hover:border-red-500/40 hover:text-red-600 dark:hover:text-red-300">
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? activeClass : cn("border-border text-muted-foreground", hoverClass),
      )}
    >
      {children}
    </button>
